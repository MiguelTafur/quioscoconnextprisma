            ...
            <dl className="mt-6 space-y-4">
                {order.orderProducts.map((orderProduct) => (
                    <div key={orderProduct.id} className="flex items-center justify-between border-t border-gray-200 pt-4">
                        <dt className="text-sm text-gray-600">{orderProduct.product.name} x {orderProduct.quantity}</dt>
                        <dd className="text-sm font-light text-gray-500">{formatCurrency(orderProduct.product.price * orderProduct.quantity)}</dd>
                    </div>
                ))}
        
                <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                    <dt className="text-base font-medium text-gray-900">Total a Pagar:</dt>
                    <dd className="text-base font-bold text-gray-900">{formatCurrency(order.total)}</dd>
                </div>
            </dl>
